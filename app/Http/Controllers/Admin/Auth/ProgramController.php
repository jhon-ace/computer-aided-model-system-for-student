    ...
    public function deleteSelected(Request $request)
    {
        ...
        if ($selectedDepartments) {
            ...
            try {
                ...
                return redirect()->route('admin.program.index')->with('success', $message);
            } catch (\Exception $e) {
                return redirect()->route('admin.program.index')->with('error', 'The following programs could not be deleted because they are associated with courses: ' 
                . implode(', ', Program::whereIn('id', $departmentsWithDeans)->pluck('program_abbreviation')->toArray()) . '.');
            }
        } else {
            return redirect()->route('admin.program.index')->with('error', 'No program/s selected for deletion.');
        }
