<div class="bg-white shadow-lg rounded-md p-5 sm:p-6 md:p-7 lg:p-8 text-black font-medium">
    ...
    <div class="flex justify-between mb-4 sm:-mt-4">
        <div class="font-bold text-md tracking-tight text-black mt-2">Admin / Manage Dean</div>
            <a href="{{ route('admin.dean.create') }}">
        ...
        <div class="overflow-x-auto">
            @if($search && $deans->isEmpty())
                <p class="text-black mt-8 text-center">No dean found for matching "{{ $search }}"</p>
            @elseif(!$search && $deans->isEmpty())
            ...
            @else
                <table class="table-auto border-collapse border border-gray-400 w-full text-center mb-4">
                    ...
                    <form id="deleteSelectedForm" action="{{ route('admin.dean.deleteSelected') }}" method="POST" onsubmit="return confirmDelete(event);">
                            @csrf
                            @method('DELETE')
                            ...
                            <tr>
                                ...
                                <td class="text-black border border-gray-400 px-4 py-2">
                                    <div class="flex justify-center items-center space-x-2">
                                        <a href="{{ route('admin.dean.edit', $dean->id) }}" class="bg-blue-500 text-white text-sm px-3 py-2 rounded hover:bg-red-500">
